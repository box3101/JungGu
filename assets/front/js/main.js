(function () {
  ...
  const comSlide = () => {
    ...
    if ($visualSlide) {
      ...
      const moVisualSlide = new Swiper('#moVisualSlide', {
        // autoplay: {
        //   delay: 3000,
        // },
        loop: true,
 ...
 const detailedSearch = () => {
  if (document.querySelector('.searchArea')) {
    ...
    $detailedSearchBtn.addEventListener("click", (e) => {
      ...
      if(target.closest(".searchArea__info").classList.contains("on")) {
          $detailedSearchBtn.setAttribute("aria-expanded","true")
          $(".searchArea__box>.hidden").text("열림");
      }
      else {
          $detailedSearchBtn.setAttribute("aria-expanded","false");
          $(".searchArea__box>.hidden").text("닫힘");
      }
    });
  }
}
  ...
  (function () {
    setTimeout(function () {
      const subHead = document.querySelector('.subHead__tit');
      const headTitle = subHead.textContent;

      document.title = `인천중구교육포털 > ${headTitle}`;
    }, 100)
  })();
})();